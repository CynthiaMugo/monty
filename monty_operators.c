...
void f_sub(stack_t **head, unsigned int counter)
{
	...
	if (nd < 2)
	{
		fprintf(stderr, "L%d: can't sb, stack too short\n", counter);
		fclose(bus.file);
		free(bus.content);
		free_stack(*head);
		exit(EXIT_FAILURE);
...
/**
 * f_div -> function that divideds the top two elements of the stack
 * @head: double head pointer to the stack
 * @counter: line count
 * Return: nothing
 */
...
/**
 * f_mod -> function that computes the remainder of the division of the 2nd top
 * element of the stack by the element of the stack
 * @head: double head pointer to the stack
 * @counter: line count
 * Return: nothing
 */
